...
const Contacts = (props) => {
    ...
    return (
        <div className="container justify-center pt-16 text-gray-50 p-4 w-full ">
            <div className="m-4">
                <button className="con justify-content rounded-full bg-red-600 p-2 text-2xl font-bold">
                    <Link to="/contactForm">
                        Create Contact +
                    </Link>
                </button>
...

                {
                    AllContacts.map((el) => {
                        return <div key={el.id} className={`rounded-lg shadow-md m-4 p-4 ${props.mode === 'dark' ? 'bg-dark text-white' : 'bg-light text-black'}`}>
                            ...
                            <div className="flex flex-col justify-between my-2 font-bold mt-4">
    <Link to={`edit/${el.id}`}>
        <button className="rounded p-2 bg-violet-600 text-black mb-2 w-full">
            Edit
        </button>
    </Link>
